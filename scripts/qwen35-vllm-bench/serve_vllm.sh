...
# Обе формы задаём явно: в vLLM префиксный кэш включён по умолчанию, поэтому
# «просто не передавать --enable-prefix-caching» его НЕ выключает.
...
get_managed_pid() {
  ...
  # Сверяемся с ИМЕНЕМ ЭТОГО файла, а не с литералом: иначе после переименования
  # скрипта рукопожатие по pid-файлу не сходится никогда.
...
usage() {
  cat <<EOF
Usage:
  ./serve_vllm.sh [VLLM_ARGS...]              Run in foreground; Ctrl+C shuts down safely
  ./serve_vllm.sh --background [VLLM_ARGS...] Start detached and write to server.log
  ./serve_vllm.sh --stop                      Gracefully stop the managed background server
  ./serve_vllm.sh --status                    Show supervisor status
  ./serve_vllm.sh --logs                      Follow the background log
