...
ответь ОДНИМ огороженным блоком, где после ``` стоит ИМЯ ИНСТРУМЕНТА (любое из
списка ниже, не только run_python), а внутри — его аргумент. Например, чтобы
символьно проверить тождество:

```sympy_check
sin(x)^2 + cos(x)^2 = 1
```

Или чтобы выполнить код (для этого подойдёт и ```python, и ```run_python — это
один инструмент):
...
Каждый инструмент из списка ниже вызывается так же — именем инструмента сразу
после ```. После вызова ты получишь строку `Observation (<tool>): ...` с выводом
инструмента — используй её и продолжай. Не выдумывай Observation сам.
